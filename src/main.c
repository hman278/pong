...
void Start() {
    ...
    racketRec =
        (Rectangle){0, 0, (float)racketTexture.width, (float)racketTexture.height / NUM_FRAMES};
...
void DrawTextureOnScreen(Texture2D texture, Rectangle source, Vector2 position, float frameCount,
                         Color color) {
    DrawTextureRec(
        texture, source,
        (Vector2){position.x - texture.width / 2.f, position.y - texture.height / 2.f / frameCount},
        color);
}
...
void Update() {
    if (IsKeyDown(KEY_W) && PlayerA.position.y >= racketTexture.height / 2.f / NUM_FRAMES) {
        PlayerA.position.y -= racketSpeed;
               ...
               PlayerA.position.y <= GetScreenHeight() - racketTexture.height / 2.f / NUM_FRAMES) {
        PlayerA.position.y += racketSpeed;
    }
    ...
    if (IsKeyDown(KEY_UP) && PlayerB.position.y >= racketTexture.height / 2.f / NUM_FRAMES) {
        PlayerB.position.y -= racketSpeed;
               ...
               PlayerB.position.y <= GetScreenHeight() - racketTexture.height / 2.f / NUM_FRAMES) {
        PlayerB.position.y += racketSpeed;
    }
