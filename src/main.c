...
void Start() {
  ...
  racketRec = (Rectangle){0, 0, (float)racketTexture.width,
                          (float)racketTexture.height / NUM_FRAMES};
  ballRec =
      (Rectangle){0, 0, (float)ballTexture.width, (float)ballTexture.height};

  // Set the initial player position
  PlayerA.position = (Vector2){20.f, GetScreenHeight() / 2.f};
  PlayerB.position =
      (Vector2){GetScreenWidth() - 20.f, GetScreenHeight() / 2.f};
  Ball.position =
...
void DrawTextureOnScreen(Texture2D texture, Rectangle source, Vector2 position,
                         float frameCount, Color color) {
  DrawTextureRec(texture, source,
                 (Vector2){position.x - texture.width / 2.f,
                           position.y - texture.height / 2.f / frameCount},
                 color);
}

void Update() {
  if (IsKeyDown(KEY_W) &&
      PlayerA.position.y >= racketTexture.height / 2.f / NUM_FRAMES) {
    PlayerA.position.y -= racketSpeed;
  } else if (IsKeyDown(KEY_S) &&
             PlayerA.position.y <=
                 GetScreenHeight() - racketTexture.height / 2.f / NUM_FRAMES) {
    PlayerA.position.y += racketSpeed;
  }

  if (IsKeyDown(KEY_UP) &&
      PlayerB.position.y >= racketTexture.height / 2.f / NUM_FRAMES) {
  ...
  if (bBallLaunched) {
    ...
    if (CheckCollisionRecs(
            ...
            (Rectangle){PlayerA.position.x + (racketRec.width / 2.f),
                        PlayerA.position.y - (racketRec.height / 2.f),
      ...
      racketRec.y = 1 * racketTexture.height / NUM_FRAMES;
               ...
               GetScreenHeight() - (ballTexture.height / 2.f)) {
      ballDir.y *= -1;
...
int main() {
  ...
  while (!WindowShouldClose()) // Detect window close button or ESC key
  {
    ...
    DrawTextureOnScreen(racketTexture, racketRec, PlayerA.position, NUM_FRAMES,
                        WHITE);
    DrawTextureOnScreen(racketTexture, racketRec, PlayerB.position, NUM_FRAMES,
                        WHITE);
